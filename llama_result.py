...
def inference(model, text, question, query_type, subject_1=None, subject_2=None):
    ...
    response = outlines.generate.choice(
        model,
        choices=choices
    )(prompt)
...
def main():
    ...
    for position in titles:
        ...
        rst_path = f"./same_gender/llama_step1/{position}_results.json"

        rst = process_contexted_questions(model, prompt_path, rst_path, K)

        with open(f"./same_gender/llama_step2/{position}_results.json", 'w') as fout:
            json.dump(rst, fout)
